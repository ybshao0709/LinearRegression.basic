...
However, the `stats::lm` function can not directly output the confidence interval(CI) of one single $\beta$ or the linear conbination of two $\beta$'s(which is often used to test the adjusted effect of a covariate in the presence of interaction terms). Therefore, we develop an user friendly R package `LinearRegression.basic`, which can be seen as an extension of the existing `stats::lm`function. The reliability and stability of this package has been fully and seriously tested.
